...
def uoc_add_round_key(state, key):
    """
    EXERCISE 1.1: AES AddRoundKey()
    :state: state matrix
    :key: AES subkey
    :return: state after AddRoundKey(). See the course materials
    """
...
def uoc_byte_sub(state, inverse=False):
    """
    EXERCISE 1.2: AES ByteSub()
    :state: state matrix
    :return: state after ByteSub(). See the course materials
    """
...
def uoc_shift_row(state, inverse=False):
    """
    EXERCISE 1.3: AES ShiftRow()
    :state: state matrix
    :return: state after ShiftRow(). See the course materials
    """
...
def uoc_mix_columns(state, inverse=False):
    """
    AES MixColumns()
    :state: state matrix
    :return: state after MixColumns(). See the course materials
    """

    #### IMPLEMENTATION GOES HERE ####

    def mix_column(column, inverse):
        c0, c1, c2, c3 = column

        if inverse:
            return [
                GF_14[c0] ^ GF_11[c1] ^ GF_13[c2] ^ GF_9[c3],
                GF_9[c0] ^ GF_14[c1] ^ GF_11[c2] ^ GF_13[c3],
                GF_13[c0] ^ GF_9[c1] ^ GF_14[c2] ^ GF_11[c3],
                GF_11[c0] ^ GF_13[c1] ^ GF_9[c2] ^ GF_14[c3]
            ]
        else:
            return [
                GF_2[c0] ^ GF_3[c1] ^ GF_1[c2] ^ GF_1[c3],
                GF_1[c0] ^ GF_2[c1] ^ GF_3[c2] ^ GF_1[c3],
                GF_1[c0] ^ GF_1[c1] ^ GF_2[c2] ^ GF_3[c3],
                GF_3[c0] ^ GF_1[c1] ^ GF_1[c2] ^ GF_2[c3]]

    for i in range(4):
        state[i] = mix_column(state[i], inverse)
...
def uoc_expand_key(key):
    """
    EXERCISE 2.2: AES Key expansion
    :state: state matrix
    :return: list of subkey matrices
    """
...
def uoc_aes_cipher(message, key):
    """
    EXERCISE 2.3: AES cipher
...
def uoc_aes_decipher(message, key):
    """
    EXERCISE 2.4: AES decipher
